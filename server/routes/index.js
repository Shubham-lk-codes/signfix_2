...
router.get('/discounted-slider-images', async (req, res) => res.json(await require('../database').getDiscountedSliderImages()));
...
router.get('/uploads/:filename', authenticate, (req,res,next)=>{const raw=req.params.filename||'';if(typeof raw!=='string'||raw.includes('\0')||raw.includes('..'))return res.status(400).json({error:'Invalid filename'});const filename=path.basename(raw);if(filename!==raw||filename.startsWith('.'))return res.status(400).json({error:'Invalid filename'});res.sendFile(path.join(uploadDir,filename),error=>{if(error&&!res.headersSent)next(Object.assign(error,{status:error.statusCode||404}))});});
