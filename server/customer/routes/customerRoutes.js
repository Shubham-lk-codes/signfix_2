...
router.patch('/addresses/:id',validate(address.partial().refine(value=>Object.keys(value).length>0,{message:'At least one address field is required'})),controller.updateAddress);
router.delete('/addresses/:id',controller.deleteAddress);
router.get('/orders/:id',controller.order);
router.post('/orders/:id/cancel',validate(z.object({reason:z.string().min(3).max(1000)})),controller.cancelOrder);
...
router.post('/services/:id/cancel',validate(z.object({reason:z.string().min(3).max(1000)})),controller.cancelService);
router.post('/services/:id/confirm-completion',validate(z.object({otp:z.string().regex(/^\d{6}$/),accepted:z.literal(true),remarks:z.string().max(1000).optional()})),controller.confirmService);
router.post('/services/:id/review',validate(z.object({rating:z.coerce.number().int().min(1).max(5),comment:z.string().max(2000).optional()})),controller.createReview);
...
router.post('/payments/intents',validate(z.object({quotationNo:z.string().min(3),type:z.enum(['advance','full']),idempotencyKey:z.string().min(8).max(120)})),controller.createPayment);
router.post('/payments/:id/capture',validate(z.object({providerPaymentId:z.string().min(3).max(160),proof:z.string().regex(/^[a-f0-9]{64}$/)})),controller.capturePayment);
router.post('/payments/:id/verify',validate(z.object({providerPaymentId:z.string().min(3).max(160),status:z.enum(['captured','failed']),proof:z.string().regex(/^[a-f0-9]{64}$/)})),controller.verifyPayment);
router.post('/payments/:id/refunds',validate(z.object({amount:z.coerce.number().positive(),reason:z.string().min(3).max(1000)})),controller.refundPayment);
router.delete('/account',validate(z.object({password:z.string().min(6)})),controller.deleteAccount);
...
const aiRequirements=z.object({businessType:z.string().max(160).optional(),signType:z.string().max(160).optional(),length:z.coerce.number().positive().optional(),width:z.coerce.number().positive().optional(),unit:z.enum(['ft','in','cm','m']).optional(),location:z.string().max(500).optional(),material:z.string().max(160).optional(),lighting:z.string().max(160).optional(),budget:z.coerce.number().nonnegative().optional(),installation:z.boolean().optional()}).partial();
router.get('/ai/config',controller.aiConfig);
router.post('/ai/chat',validate(z.object({message:z.string().min(1).max(4000),requirements:aiRequirements.optional()})),controller.aiChat);
router.get('/ai/conversations',controller.conversations);
router.post('/ai/leads',validate(z.object({requirement:z.string().min(3),product:z.string().optional(),budget:z.coerce.number().nonnegative().optional(),contact:z.string().min(5),requirements:aiRequirements.optional()})),controller.createLead);
