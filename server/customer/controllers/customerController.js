...
async function aiChat(req,res){const reply=makeReply(req.body.message);
    const escalate=/human|support|agent|escalat/i.test(req.body.message);const saved=await repo.saveConversation(req.user.id,req.body.message,reply,escalate);res.json({...saved,name:'SignFix AI Assistant',reply,actions:['Calculate Price','Request Design','Talk to Support'],disclaimer:'AI guidance and designs are concepts only. Final quotation, feasibility and delivery dates require Admin approval.'});}
